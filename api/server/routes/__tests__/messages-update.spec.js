...
/**
 * The route's job is to hand the edited message to the retention helper; whether that
 * helper writes is decided by the retention mode and covered by its own unit test in
...
describe('PUT /:conversationId/:messageId', () => {
  ...
  const expectRestamp = (ctx) =>
    expect(applyForcedRetention).toHaveBeenCalledWith(
      { saveMessage, saveConvo },
      { ctx: expect.objectContaining(ctx), conversationId, messageId, context },
    );
  ...
  it('carries the stored deadline of an already-temporary message', async () => {
    const expiredAt = new Date('2030-01-01T00:00:00.000Z');
    getMessages.mockResolvedValue([
      { conversationId, quotes: [], isCreatedByUser: true, isTemporary: true, expiredAt },
    ]);
    ...
    expect(response.status).toBe(200);
    expectRestamp({ userId, isTemporary: true, expiredAt });
  });
